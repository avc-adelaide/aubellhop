...
* `.to_file(fname)` --- write environment configuration to file(s) in original Bellhop `.env` text format (also writing additional `.ssp`, `.bty` files if needed)
...
## Reading and writing --- `.read()` & `.to_file()`
